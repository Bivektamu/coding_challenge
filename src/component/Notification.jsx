...
const Notification = ({ allNots }) => {

    const [cs, setCs] = useState(' duration-0')
    const ref = useRef(null)
    const [notArr, setNotArr] = useState(allNots)
    ...
    const closeFunc = (id) => {
        const filteredArr = notArr.filter((n, i) => i !== 0)
        ...
        <div className='self-end gap-y-3 flex flex-col'>
            {notArr.map((n, i) => {
                ...
                switch (n) {
                    case 'error':
                        csName = 'red'
                        svg = (<svg xmlns="http://www.w3.org/2000/svg" width="25" height="25" viewBox="0 0 25 25" fill="none" className="fill-red-500 aspect-square object-contain object-center w-[25px] fill-black fill-opacity-0 overflow-hidden shrink-0 max-w-full">
                    ...
                    default:
                        csName = 'green'
                        svg = (<svg className='fill-green-500' xmlns="http://www.w3.org/2000/svg" width="25" height="25" viewBox="0 0 25 25" fill="none">
                ...
                return (
                    <div className='self-end' >
                        <div className={`border-b-${csName}-500 shadow-lg relative bg-white flex w-[439px] max-w-full items-stretch justify-between gap-5 px-4 py-7 rounded-lg  self-end max-md:flex-wrap`}>
                            ...
                            <svg ref={ref} onClick={() => closeFunc(i)} xmlns="http://www.w3.org/2000/svg" width="15" height="15" viewBox="0 0 15 15" fill="none" className="cursor-pointer aspect-square object-contain object-center w-[15px] fill-indigo-300 overflow-hidden self-center shrink-0 max-w-full my-auto">
